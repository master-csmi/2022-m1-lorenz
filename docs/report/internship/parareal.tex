...
\begin{algorithm*}
	...
	\Input{$t_0$ : initial time ; \quad $T$ : final time ; \quad $U_0^0$ : initial point ; \\
		$F$ : fine integrator ; \quad $G$ : coarse integrator ; \\
		$\Delta t_F$ : fine time step ; \quad $\Delta t_G$ : coarse time step ; \\
...
\begin{algorithm}[H]
	...
	k$\leftarrow$0\;
	init\_pts$\leftarrow$[$U_0^0$]\;
	coarse$\leftarrow$[]\;
	fine$\leftarrow$[]\;
	\For{$j\leftarrow 1$ \KwTo $P$}{
		coarse[j-1]$\leftarrow$G(init\_points[j-1])[-1]\;
		init\_pts[j]$\leftarrow$coarse[j]\;
	}
	\textcolor{violet}{$U_j^k \leftarrow$ send init\_pts[j] to each proc j\;}
	\textcolor{blue}{\DoParallel{
		tab\_F$\leftarrow F(U_j^k)$ \tcp{fine solution on each sub-interval}
	}}
	\textcolor{violet}{fine[j]$\leftarrow$ recv tab\_F[-1] from each proc j\;}
	\caption{initialization}\label{initialization}
\end{algorithm}\DecMargin{1em}

\newpage
\noindent And this is the entire algorithm for the parareal method (with the initialization) :
...
\begin{algorithm}[H]
	...
	\While{not converge}{
		\For{$j\leftarrow 1$ \KwTo $P$}{
			val\_G$\leftarrow$G(init\_points[j-1])[-1]\;
			init\_pts[j]$\leftarrow$val\_G+fin[j-1]-coarse[j-1]\;
			coarse[j-1]$\leftarrow$val\_G\;
		}
		\textcolor{violet}{$U_j^k \leftarrow$ send init\_pts[j] to each proc j\;}
		\textcolor{blue}{\DoParallel{
				tab\_F$\leftarrow F(U_j^k)$ \tcp{fine solution on each sub-interval}
	...
	\tcp{Get the solution between $y_0$ and $T$}
	\tcp{$sol(t_0,T)=[sol(t_0,t_1),\dots,sol(t_{P-1},t_P)]$}
	\textcolor{violet}{sol $\leftarrow$ send the final point from the last process\;}
	\return sol
	\caption{parareal}\label{parareal}
\end{algorithm}\DecMargin{1em}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.3\linewidth]{"images/parareal/cpp/osci_cvg_1.jpg"}}
