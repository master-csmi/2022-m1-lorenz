...
def parareal_method(X0_t0,t0,T,fct,dt_G,dt_F,gamma=None): 
    # fine integrator
    ...
    if(rank==0):
        sol_k = sol_k.reshape((-1,len(X0_t0)))
        solutions_x.insert(len(solutions_x.columns),'k=0',sol_k[:,0])
        solutions_y.insert(len(solutions_y.columns),'k=0',sol_k[:,1])
        solutions_z.insert(len(solutions_z.columns),'k=0',sol_k[:,2])
...
parareal_method(X0,t0,T,lorenz,dt_G,dt_F,gamma)
